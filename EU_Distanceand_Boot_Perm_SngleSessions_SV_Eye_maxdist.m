...
Dist_LowHigh_Eye_1_boot = nan(length(Sess_numb), nAreas);   % SAME
Dist_LowHigh_Eye_2_boot = nan(length(Sess_numb), nAreas);   % OTHER
...
sess_labels = find(Eye.Session==b);

EyeLowSess= intersect (Eye.StimLow,sess_labels);
EyeHighSess= intersect (Eye.StimHigh,sess_labels);

Left  = intersect(Eye.Left,sess_labels);
Right = intersect(Eye.Right,sess_labels);

idx_LL0 = intersect ( EyeLowSess,  Left );
idx_LR0 = intersect ( EyeLowSess,  Right );
idx_HL0 = intersect ( EyeHighSess, Left );
idx_HR0 = intersect ( EyeHighSess, Right );
 ...
 for ar = 1:nAreas
        ...
        % ---------------------------------------------------------
        x_LL = nanmean(Proj_1_B_Dir_Eye_All(idx_LL0, 5:60, ar), 1);
        y_LL = nanmean(Proj_1_B_SV_Eye_All (idx_LL0, 5:60, ar), 1);

        x_LR = nanmean(Proj_1_B_Dir_Eye_All(idx_LR0, 5:60, ar), 1);
        y_LR = nanmean(Proj_1_B_SV_Eye_All (idx_LR0, 5:60, ar), 1);

        x_HL = nanmean(Proj_1_B_Dir_Eye_All(idx_HL0, 5:60, ar), 1);
        y_HL = nanmean(Proj_1_B_SV_Eye_All (idx_HL0, 5:60, ar), 1);

        x_HR = nanmean(Proj_1_B_Dir_Eye_All(idx_HR0, 5:60, ar), 1);
        y_HR = nanmean(Proj_1_B_SV_Eye_All (idx_HR0, 5:60, ar), 1);
...
nAreas = size(Proj_1_B_SV_Eye_All,3);
...
% ------------------------------------------------------------
    ...
    for ar = 1:nAreas
        ...
        % x = choice direction, y = action value
        % ---------------------------------------------------------
        x_LL = nanmean(Proj_1_B_Dir_Eye_All(idx_LL, window_size_L(ar)-Bin_Ext:window_size_L(ar)+Bin_Ext, ar), 1);
        y_LL = nanmean(Proj_1_B_SV_Eye_All (idx_LL,  window_size_L(ar)-Bin_Ext:window_size_L(ar)+Bin_Ext, ar), 1);

        x_LR = nanmean(Proj_1_B_Dir_Eye_All(idx_LR,  window_size_R(ar)-Bin_Ext:window_size_R(ar)+Bin_Ext, ar), 1);
        y_LR = nanmean(Proj_1_B_SV_Eye_All (idx_LR,  window_size_R(ar)-Bin_Ext:window_size_R(ar)+Bin_Ext, ar), 1);

        x_HL = nanmean(Proj_1_B_Dir_Eye_All(idx_HL,  window_size_L(ar)-Bin_Ext:window_size_L(ar)+Bin_Ext, ar), 1);
        y_HL = nanmean(Proj_1_B_SV_Eye_All (idx_HL,  window_size_L(ar)-Bin_Ext:window_size_L(ar)+Bin_Ext, ar), 1);

        x_HR = nanmean(Proj_1_B_Dir_Eye_All(idx_HR,  window_size_R(ar)-Bin_Ext:window_size_R(ar)+Bin_Ext, ar), 1);
        y_HR = nanmean(Proj_1_B_SV_Eye_All (idx_HR,  window_size_R(ar)-Bin_Ext:window_size_R(ar)+Bin_Ext, ar), 1);
        ...
        % ---------------------------------------------------------
        x_LL = nanmean(Proj_1_B_Dir_Eye_All(idx_LL,  window_size_L(ar)-Bin_Ext:window_size_L(ar)+Bin_Ext, ar), 1);
        y_LL = nanmean(Proj_2_B_SV_Eye_All (idx_LL,  window_size_L(ar)-Bin_Ext:window_size_L(ar)+Bin_Ext, ar), 1);

        x_LR = nanmean(Proj_1_B_Dir_Eye_All(idx_LR,  window_size_R(ar)-Bin_Ext:window_size_R(ar)+Bin_Ext, ar), 1);
        y_LR = nanmean(Proj_2_B_SV_Eye_All (idx_LR,  window_size_R(ar)-Bin_Ext:window_size_R(ar)+Bin_Ext, ar), 1);

        x_HL = nanmean(Proj_1_B_Dir_Eye_All(idx_HL,  window_size_L(ar)-Bin_Ext:window_size_L(ar)+Bin_Ext, ar), 1);
        y_HL = nanmean(Proj_2_B_SV_Eye_All (idx_HL,  window_size_L(ar)-Bin_Ext:window_size_L(ar)+Bin_Ext, ar), 1);

        x_HR = nanmean(Proj_1_B_Dir_Eye_All(idx_HR,  window_size_R(ar)-Bin_Ext:window_size_R(ar)+Bin_Ext, ar), 1);
        y_HR = nanmean(Proj_2_B_SV_Eye_All (idx_HR,  window_size_R(ar)-Bin_Ext:window_size_R(ar)+Bin_Ext, ar), 1);
        ...
        % Store
        Dist_LowHigh_Eye_1_boot(b, ar) = D_same;
        Dist_LowHigh_Eye_2_boot(b, ar) = D_other;
...
%%
% ============================================================
% SUMMARY FOR BARPLOT (16 bars: same/other per area)
% (Use bootstrap means + bootstrap SEM for visualization)
% ============================================================
for a= 1:8
nsess_areas(a) = numel(find(~isnan(Dist_LowHigh_Eye_1_boot(:,a))==1));
end
mean1 = mean(Dist_LowHigh_Eye_1_boot, 1, 'omitnan');   % SAME
mean2 = mean(Dist_LowHigh_Eye_2_boot, 1, 'omitnan');   % OTHER

std_1  = std(Dist_LowHigh_Eye_1_boot, [], 1, 'omitnan')./sqrt(nsess_areas);
std_2  = std(Dist_LowHigh_Eye_2_boot, [], 1, 'omitnan')./sqrt(nsess_areas);
...
figure('Color','w','Position',[200 200 520 300]); hold on;

b = bar(x_value, bArmeans, 'FaceColor','flat');

% SAME = orange, OTHER = blue
b.CData(1:2:end,:) = repmat([0.85 0.33 0.10], nAreas, 1);
b.CData(2:2:end,:) = repmat([0.30 0.75 0.93 ], nAreas, 1);

errorbar(x_value, bArmeans, barSEMs, 'k', ...
    'LineStyle','none','LineWidth',1,'CapSize',8);

% xticks(x_value);
xticklabels(reshape([areaLabels areaLabels]', 1, []));
xtickangle(45);
xlim([min(x_value)-1, max(x_value)+1]);
...
%         axis square
grid off
ylabel('Low–High Value Distance');
% title(['Arm trials: ' num2str(time_u(window_size(1))) ' ' num2str(time_u(window_size(end)))]);
title('Stimulus value - eye');
...
diff_boot = Dist_LowHigh_Eye_1_boot - Dist_LowHigh_Eye_2_boot;
[h,p] = ttest(Dist_LowHigh_Eye_1_boot,Dist_LowHigh_Eye_2_boot);
...
% ---------------- Plot (SD version) ----------------
figure('Color','w','Position',[300 300 520 320]); hold on;

% Zero line: no generalization gap
yline(0, 'k--', 'LineWidth', 1);

% Error bars (bootstrap SD)
errorbar(x, sepIdx, sepIdxSD, 'k', ...
    'LineStyle','none', 'LineWidth', 1, 'CapSize', 8);

% Central points
scatter(x, sepIdx, 55, 'filled', ...
...
%         axis square
        grid off
ylim([0 .7])
% ylabel('Generalization gap (within − cross)
title('Stimulus value - eye');
...
for area = 1:8
area_angle=[];
area_angle = Angles_all_unfolded (:,var_angle, area);
...
subplot(2,4,area)
plot(valid_area_angle,Matched_Dist,'.','Color','r'); hold on
X = valid_area_angle;
Y = Matched_Dist;
 mdl = fitlm(X,Y);
            ...
            plot(xx, yy, 'r','LineWidth',.75); hold on
%             plot(xx, yCI(:,1), 'r--')
%             plot(xx, yCI(:,2), 'r--')

            R = corr(X,Y,'rows','complete');
            text(0.05, 0.95, sprintf('r = %.2f',R), ...
                 'Units','normalized','FontSize',10,'FontWeight','normal','Color','r'); hold on
...
title('Eye - Stim value');


box off;
f = gcf; 
            filename = strcat('Eye - Stim value','.pdf');
...
nSessions = size(Dist_LowHigh_Eye_1_boot, 1);
...
for ar = 1:nAreas
    for s = 1:nSessions
        ...
        % Matched condition
        d1 = Dist_LowHigh_Eye_1_boot(s, ar);
        if ~isnan(d1)
            dist_vec(end+1)   = d1;
        ...
        % Cross condition
        d2 = Dist_LowHigh_Eye_2_boot(s, ar);
        if ~isnan(d2)
            dist_vec(end+1)   = d2;
...
for ar = 1:nAreas
    d1    = Dist_LowHigh_Eye_1_boot(:, ar);
    d2    = Dist_LowHigh_Eye_2_boot(:, ar);
...
Sim_behavSess = [ 11  15  34 35 37 45  49  50  ]; % selected 

Dist_SimBehav_1 = Dist_LowHigh_Eye_1_boot (Sim_behavSess,:) ; 
Dist_SimBehav_2 = Dist_LowHigh_Eye_2_boot (Sim_behavSess,:) ; 
...
title('Eye - Stim value same behav');
...
% ── Data summary ──────────────────────────────────────────────────────
fprintf('\n=== Cross-system readout — Similar behavior sessions Saccade ===\n')
...
fprintf('\nSimilar behavior sessions statistics Saccade done.\n')
...
for ar = 1:nAreas
    for s = 1:size(Dist_LowHigh_Eye_1_boot, 1)
...
% ── Data summary ──────────────────────────────────────────────────────
fprintf('\n=== Cross-system readout with Behavioral Similarity Saccade===\n')
fprintf('Total observations:   %d\n', length(dist_vec_bs))
...
for bv = {'Similar', 'Different'}
    ...
    if strcmp(bv{1}, 'Similar')
        ...
        sessSet = setdiff(1:size(Dist_LowHigh_Eye_1_boot,1), Sim_behavSess);
    end
    ...
    for ar = 1:nAreas
        d1 = Dist_LowHigh_Eye_1_boot(sessSet, ar);
        d2 = Dist_LowHigh_Eye_2_boot(sessSet, ar);
        valid = ~isnan(d1) & ~isnan(d2);
...
fprintf('\nCross-system readout all sessions behavioral similarity analysis Saccade done.\n')