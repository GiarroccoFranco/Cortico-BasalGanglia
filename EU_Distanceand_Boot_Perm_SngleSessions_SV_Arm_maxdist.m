%% ============================================================
%  Arm ONLY — LOW vs HIGH distance (Left+Right separately) + VALID NULL TEST
%  Replaces your script, but keeps the same structure/variables.
%
%  Key changes vs your current code:
%   (1) Distance computed separately for LEFT and RIGHT:
%       D = mean( norm(HL-LL), norm(HR-LR) )
%   (2) No t-test on bootstraps. Instead: permutation/sign-flip null on Delta
%       Delta = D_same - D_other (one value per bootstrap)
%       p(one-sided) = P(Delta_null >= Delta_obs)
%
%  Outputs:
%     Dist_LowHigh_Arm_1_boot  [nBoot x nAreas]  % SAME (real space)
%     Dist_LowHigh_Arm_2_boot  [nBoot x nAreas]  % OTHER (cross space)
%     Delta_boot              [nBoot x nAreas]  % same - other, per bootstrap
%     Delta_mean              [1 x nAreas]
%     pvals_perm              [1 x nAreas]      % valid p-values
%     bar plot: 16 bars (same/other per area) + stars (perm p<0.05)
%% ============================================================
...
% ------------------------------------------------------------
    ...
    % ---- bootstrap resample WITH replacement (size = original N) ----
    idx_LL = idx_LL0;
    idx_LR = idx_LR0;
    idx_HL = idx_HL0;
    idx_HR = idx_HR0;
...
b.CData(1:2:end,:) = repmat([0.30 0.75 0.93], nAreas, 1);
b.CData(2:2:end,:) = repmat([0.85 0.33 0.10], nAreas, 1);
...
% (use mean to stay consistent with your earlier code)
...
subplot(2,4,area)
plot(valid_area_angle,Matched_Dist,'.','Color','b'); hold on
X = valid_area_angle;
Y = Matched_Dist;
 mdl = fitlm(X,Y);
            ...
            plot(xx, yy, 'b','LineWidth',.75); hold on
%             plot(xx, yCI(:,1), 'r--')
%             plot(xx, yCI(:,2), 'r--')

            R = corr(X,Y,'rows','complete');
            text(0.05, 0.80, sprintf('r = %.2f',R), ...
                 'Units','normalized','FontSize',10,'FontWeight','normal','Color','b'); hold on
...
box off;
f = gcf; 
            filename = strcat('Arm -SV value','.pdf');
...
% ── Data summary ──────────────────────────────────────────────────────
fprintf('\n=== Cross-system readout — Similar behavior sessions REACH ===\n')
...
fprintf('\nSimilar behavior sessions statistics REACH done.\n')
...
% ── Data summary ──────────────────────────────────────────────────────
fprintf('\n=== Cross-system readout with Behavioral Similarity Reach===\n')
fprintf('Total observations:   %d\n', length(dist_vec_bs))
...
fprintf('\nCross-system readout all sessions behavioral similarity analysis REACH done.\n')
%%
